#' Decision Function for 2 Sample Operating Characteristics
#'
...
oc2Sdecision <- function(pc=0.975, qc=0, lower.tail=TRUE, link=c("identity", "logit", "log")) {
    ...
    fun <- function(post1, post2, dist=FALSE) {
        dlink(post1) <- dlink_obj
        dlink(post2) <- dlink_obj
        ...
        test <- if(inherits(post1, "normMix"))
                    pmix(mixnormdiff(post1, post2), qc, lower.tail=lower.tail, log.p=TRUE) - lpc
                else
                    log(pmax(pmixdiff(post1, post2, qc, lower.tail=lower.tail), .Machine$double.eps) ) - lpc
        if(dist)
            return(test)
    ...
    class(fun) <- c("oc2Sdecision", "function")
    fun
}

#' @export
print.oc2Sdecision <- function(x, ...) {
    ...
    for(i in seq_along(qc)) {
        cat(paste0("P(x1 - x2 ", cmp, " ", qc[i], ") > ", pc[i], "\n"))
    }
    cat("Link:", link, "\n")
    invisible(x)
}
