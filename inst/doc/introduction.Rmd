...
RBesT with the **`ess`** function. It should be noted, however, that the
concept of ESS is somewhat elusive. In particular, the definition of
the ESS is not unique and multiple methods have therefore been
implemented in RBesT. The default moment-based approach yields
conservative (small) ESS in comparison to the Morita [3] approach,
which yields liberal (large) ESS:
...
by using few mixture components for the MAP MCMC sample:
...
Note that for a 1-sample situation the respective `oc1Sdecision` and
...
**`oc2S`** help for more information.
...
[2] Beaten D. et. al, _The Lancet_, 2013, (382), 9906, p 1705  
